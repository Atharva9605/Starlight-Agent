...
def get_kb_status():
    """Returns number of chunks and list of catalogue names in the vector store."""
    return get_status()
    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            recipient = (
                ...
                if st.session_state.recipient_override
                else scrape_data.get("emails", row.get("Email", ""))
            )
            send_result = send_email_gsuite(
                eml_path,
                sender_email=st.session_state.sender_email,
            )
            
            if send_result.get("success") if isinstance(send_result, dict) else send_result:
