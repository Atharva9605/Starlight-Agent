...
def invoke_chain_get_text(chunk):
    ...
    messages = [
        {"role": "system", "content": get_prompt("scraper_system")},
...
def analyze_text_with_llm(full_text):
    from schemas import CompanyProfile, parse_with_retry
    ...
    def _call():
        return invoke_chain_get_text(truncated)
        ...
        return azure_manager.chat_completion(
            [
                {"role": "system", "content": get_prompt("scraper_system")},
                {
                    ...
                    "content": (
                        f"Website text:\n\n{truncated}\n\n"
                        "Return ONLY valid JSON with keys: summary, products_services, "
                        "target_audience, usp, key_phrases, tone_style."
                    ),
                },
            ],
            temperature=0.0,
    ...
    try:
        profile = parse_with_retry(CompanyProfile, raw_out, retry_fn=_retry)
