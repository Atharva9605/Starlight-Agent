...
Pipeline (per page):
  ...
  7. Embed documents with Azure OpenAI (deployment from AZURE_OPENAI_EMBEDDING_DEPLOYMENT)
  8. Upsert into vector store collection "starlight_vision"
...
Guidelines:
- Please extract only values that are visibly printed on this page.
...
def ingest_catalogue(
    ...
    if force_reingest:
        ...
        if deleted:
            log.info("Deleted %d stale entries for %s", deleted, filename)
